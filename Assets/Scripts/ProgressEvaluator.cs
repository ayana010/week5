...
public class ProgressEvaluator : MonoBehaviour
{
    ...
    [SerializeField] private GradeBand Deactivate = GradeBand.Deactivate;
    ...
    void Start()
    {
        ...
        try
        {
            if (Deactivate > GradeBand.Deactivate)
            {
                ...
                evalMethods[(int)Deactivate - 1]();
                if (Deactivate <= GradeBand.Pass60)
                {
                    EvalPassMessage(Deactivate.ToString());
                    if (Deactivate > loadedBand)
                        EvalOutput(Deactivate);
                }
                else
                    EvalInProgressMessage(Deactivate.ToString());
            }
        ...
        catch (EvalFailedException e)
        {
            ...
            EvalFailMessage(Deactivate.ToString(), e.Message);
        }
        catch (Exception e)
        {
            // EvalFailMessage(((GradeBand)i + 1).ToString()
            EvalFailMessage(Deactivate.ToString(), "An unknown error occured during the progress evaluation. It is likely " +
    ...
    private IEnumerator Dist80Coroutine()
    {
        ...
        EvalInstructions(Deactivate.ToString(), "Press the Q key 4 times to complete this evaluation");
        int i = 0;
        while (i < 4)
        {
    ...
    {
        yield return null;
        EvalInstructions(Deactivate.ToString(), "Press the Q key 1 time to complete this evaluation");
    ...
    {
        yield return null;
        EvalInstructions(Deactivate.ToString(), "Press the E key 2 times to complete this evaluation");
